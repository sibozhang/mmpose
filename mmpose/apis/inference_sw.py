...
def _inference_single_pose_model(model,
    ...
    if dataset in ('TopDownCocoDataset'):
        flip_pairs = []
    elif dataset in ('TopDownOCHumanDataset',
...
def vis_pose_result(model,
    ...
    if dataset == 'TopDownCocoDataset':
        palette = np.array([[0, 0, 0], [0, 255, 0], [0, 0, 255], [255, 0, 0], [255, 255, 0],
                        [255, 255, 255]])
        # show the results
        skeleton = [[1, 3], [2, 3], [3, 4], [4, 5], [5, 6]]
    ...
    elif dataset in ('BottomUpCocoDataset',
